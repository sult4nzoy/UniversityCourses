...
void maze::dfs()
{
    ...
    while (!S.empty())
    {
        ...
        {
            S.pop();
            if (!(solved.top()->x == exitNode->x && solved.top()->y == exitNode->y))
            {
                solved.pop();
            }
        }
...
std::vector<node *> maze::getUnvisitedNeighbors(node *currentNode)
{
    ...
    if (x > 0 && (!getNode(x - 1, y)->visited))
    {
        neighbors.push_back(getNode(x - 1, y)); // vänster
    }
    if (x < width - 1 && (!getNode(x + 1, y)->visited))
    {
        neighbors.push_back(getNode(x + 1, y)); // höger
    }
    if (y > 0 && (!getNode(x, y - 1)->visited))
    {
        neighbors.push_back(getNode(x, y - 1)); // upp
    }
    if (y < height - 1 && (!getNode(x, y + 1)->visited))
    {
        neighbors.push_back(getNode(x, y + 1)); // ner
    }
...
void maze::visualizeMaze()
{
    ...
    for (int x = 0; x < visWidth; x++)
    {
        for (int y = 0; y < visHeight; y++)
        {
            ...
            else if (x == exitNode->x * 2 + 1 && y == exitNode->y * 2 + 2)
            {
